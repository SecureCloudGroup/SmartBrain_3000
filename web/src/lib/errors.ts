...
//
// A 423 (vault locked) or a 401 no_session returns "" — api.ts has already redirected
// to /unlock, so callers should NOT paint an error for it (avoids a flash before navigation).
//
...
export function describeError(err: unknown): string {
  ...
  const s = err.status;
  if (s === 423 || err.code === "no_session") return ""; // api.ts already navigated to /unlock
