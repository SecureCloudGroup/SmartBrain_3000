...
{#if account.status?.unlocked}
  ...
  {#if pending.length > 0}
    ...
    {#each pending as p (p.id)}
      <ActionCard icon={iconForTool(p.tool)} title={p.tool} tier={p.tier === "irreversible" ? "irreversible" : "reviewed"} promoted={promotedLine(p.tool, p.args, p.mcp_label, p.composite_titles, p.recipe_url) ?? ""} scope={fmtArgs(p.args)}>
        {#snippet actions()}
