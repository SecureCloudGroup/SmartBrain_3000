...
const sections = files.map((file) => {
  ...
  const headingTokens = marked.lexer(md).filter((t) => t.type === "heading");
  const headingSlugs = headingTokens.map((t) => slugify(t.text));
  // The page's own table of contents for the Help sidebar: its "##" sections. A reader
  // looking for "Voice" or "Settings" should see the word without opening every page.
  ...
  return { slug, title, headings, html };
});
...
const body = `export interface DocHeading { id: string; text: string; }\n` +
  `export interface DocSection { slug: string; title: string; headings: DocHeading[]; html: string; }\n` +
