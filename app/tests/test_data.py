...
def test_upgrade_from_v12_preserves_all_client_data(tmp_path) -> None:
    ...
    applied = dbmod.run_migrations(conn)
    assert applied == 27  # exactly ids 13..39 (37 UTC shift; 38 feeds; 39 usage cost)
...
def test_upgrade_from_v2_preserves_documents(tmp_path) -> None:
    ...
    assert dbmod.run_migrations(conn) == 37  # ids 3..39
...
def test_upgrade_from_v17_preserves_embeddings_and_tasks(tmp_path) -> None:
    ...
    assert dbmod.run_migrations(conn) == 22  # ids 18..39
...
def test_migration_19_marks_preexisting_runs_seen(tmp_path) -> None:
    ...
    assert dbmod.run_migrations(conn) == 21  # 19 seen; 20-23 vaults; 24 summaries; 25 trash; 26 embed total; 27-35 selfimprove; 36 import traces; 37 UTC shift; 38 feeds; 39 usage cost
...
def test_app_boots_and_serves_data_after_v12_upgrade(tmp_path, monkeypatch) -> None:
    ...
    assert dbmod.run_migrations(conn) == 27  # ids 13..39 (vaults + summaries + trash + embed total + selfimprove + import traces + UTC shift + feeds + usage cost)
