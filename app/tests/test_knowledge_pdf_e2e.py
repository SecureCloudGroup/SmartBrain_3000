...
# --- a real, minimal PDF the app's pypdf can read ------------------------------------------

def make_pdf(pages_text: list[str]) -> bytes:
    """Build a minimal multi-page PDF (Helvetica) whose text pypdf can extract. Test-only."""
...
def _long_pages(marker: str, pages: int = 6, lines_per_page: int = 18) -> list[str]:
...
def test_multichunk_pdf_late_token_is_embedded_and_found(monkeypatch) -> None:
    ...
    _use_fake_embed(monkeypatch)
    kb, conn, _ = _kb()
    _, text = ingest.from_file("long.pdf", make_pdf(_long_pages("MUNGWANGLE")))
