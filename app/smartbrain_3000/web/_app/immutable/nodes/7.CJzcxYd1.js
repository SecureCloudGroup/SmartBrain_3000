import{A as e,E as t,H as n,J as r,K as i,M as a,O as o,Q as s,R as c,T as l,U as u,Y as d,Z as f,at as p,c as m,h,k as g,ot as _,q as v,tt as y,ut as b,x}from"../chunks/Dpf1ywhm.js";import"../chunks/xihTtKlq.js";import{t as S}from"../chunks/CLwGJCKi.js";var C=[{slug:`getting-started`,title:`Getting started`,headings:[{id:`what-you-need`,text:`What you need`},{id:`install`,text:`Install`},{id:`first-run`,text:`First run`},{id:`your-first-5-minutes`,text:`Your first 5 minutes`},{id:`locking-and-unlocking`,text:`Locking and unlocking`},{id:`updating`,text:`Updating`},{id:`troubleshooting`,text:`Troubleshooting`},{id:`uninstall`,text:`Uninstall`},{id:`next`,text:`Next`}],html:`<h1 id="getting-started">Getting started</h1>
...
(<a href="http://localhost:33000">http://localhost:33000</a>); to reach it from your other devices see
...
<p>Each browser still proves itself once. A browser that hasn&#39;t opened SmartBrain since the
app last started asks for your passphrase (<strong>Open SmartBrain here</strong>) even when the vault
is already unlocked; after that it follows the shared lock like any other screen.</p>
<ul>
...
`},{slug:`models`,title:`Connect a model`,headings:[{id:`cloud-providers-your-api-keys`,text:`Cloud providers (your API keys)`},{id:`local-models-yours-on-this-machine-or-another-one-you-own`,text:`Local models (yours — on this machine or another one you own)`},{id:`claude-code-sends-your-chats-to-anthropic`,text:`Claude Code (sends your chats to Anthropic)`},{id:`choosing-a-model-in-chat`,text:`Choosing a model in Chat`},{id:`which-model-does-what-model-routing`,text:`Which model does what (Model routing)`},{id:`embeddings-for-knowledge-search`,text:`Embeddings (for Knowledge search)`},{id:`voice-dictation-and-spoken-replies`,text:`Voice (dictation and spoken replies)`},{id:`next`,text:`Next`}],html:`<h1 id="connect-a-model">Connect a model</h1>
...
<h2 id="claude-code-sends-your-chats-to-anthropic">Claude Code (sends your chats to Anthropic)</h2>
<p>If you already pay for Claude (any Claude plan, or an API sign-in through the Claude
Code app), SmartBrain can use Anthropic&#39;s models <strong>through the <code>claude</code> command
installed on this computer</strong> — no API key to create or paste. It&#39;s configured on the
same page as your local servers because it behaves like one operationally, but it is
<strong>not local in the privacy sense</strong>:</p>
<blockquote>
<p><strong>This option sends your chats to Anthropic.</strong> Your messages — and any knowledge or
...
<p>Set it up (Settings → Local models → <strong>Claude Code</strong>):</p>
<ol>
<li><strong>Install Claude Code</strong> — run the command shown on the page
(<code>curl -fsSL https://claude.ai/install.sh | bash</code>, or <code>brew install --cask claude-code</code>).</li>
<li><strong>Sign in</strong> — open a terminal, run <code>claude</code>, and sign in with your Claude account
when the browser opens. SmartBrain never sees or stores that sign-in; it stays with
the <code>claude</code> command, and SmartBrain doesn&#39;t know or care which plan you&#39;re on.</li>
<li>Back in SmartBrain, press <strong>Check again</strong> — the page verifies the command is
installed and signed in (a local check only; no chat is sent) — then <strong>Connect</strong>.</li>
</ol>
<p>Three models appear in the pickers and under Model routing: <code>claudecode/opus</code>,
<code>claudecode/sonnet</code>, and <code>claudecode/haiku</code> (aliases — the <code>claude</code> command resolves
each to its current generation, so they stay valid across updates).</p>
<h3 id="how-it-stays-contained">How it stays contained</h3>
<p>Claude Code is normally an <em>agent</em> that can read files and run commands. SmartBrain
does not use it that way: it drives the <code>claude</code> command in plain-text mode with a
<strong>custom agent whose tool set is empty</strong> (<code>&quot;tools&quot;: []</code>) and <strong>session persistence
off</strong>. That combination turns the CLI into a pure language-model endpoint — it cannot
...
none of your Claude Code settings or <code>CLAUDE.md</code> files, so nothing you&#39;ve written
...
account (your own — Anthropic already has it). Pressing <strong>Remove</strong> on the card
truly disconnects: SmartBrain refuses to serve <code>claudecode/*</code> models afterwards,
even if a routing entry or schedule still names one. (These invariants are
enforced by tests in SmartBrain&#39;s suite, not just promised here.)</p>
<p>Claude Code installed natively keeps itself up to date; the <strong>Update Claude Code</strong>
button on the page checks and installs right now. Docker installs can&#39;t use this
provider — the <code>claude</code> command lives on the host, so run SmartBrain natively.</p>
...
`},{slug:`features`,title:`Using SmartBrain_3000`,headings:[{id:`where-things-live`,text:`Where things live`},{id:`chat`,text:`Chat`},{id:`voice`,text:`Voice`},{id:`what-the-assistant-can-do`,text:`What the assistant can do`},{id:`knowledge`,text:`Knowledge`},{id:`planner`,text:`Planner`},{id:`neural-interface`,text:`Neural Interface`},{id:`schedules`,text:`Schedules`},{id:`info`,text:`Info`},{id:`email-gmail`,text:`Email (Gmail)`},{id:`memory`,text:`Memory`},{id:`web-search`,text:`Web search`},{id:`self-improvement`,text:`Self-improvement`},{id:`usage-cost`,text:`Usage & cost`},{id:`activity`,text:`Activity`},{id:`settings`,text:`Settings`},{id:`next`,text:`Next`}],html:`<h1 id="using-smartbrain_3000">Using SmartBrain_3000</h1>
...
<td>Everything you configure — and <strong>Status</strong>, a live view of everything the app is doing. Desktop only. Tab by tab under <a href="#settings">Settings</a>, below.</td>
</tr>
</tbody></table>
...
reply short, press <strong>Stop</strong>. (The microphone stays closed while a reply is read: it
would hear the reply too, and an open microphone makes the system turn the voice
down — so the mic opens the moment the voice finishes.)</li>
...
<h2 id="neural-interface">Neural Interface</h2>
<p>A dashboard of things you asked to watch. Type what you want into the box at
the top of the <strong>Neural</strong> page — <em>&quot;show me AAPL every 5 minutes&quot;</em>, <em>&quot;tide
times for Wallace Creek&quot;</em> — and a deterministic build engine turns your words
...
schedule. (Chat can <em>read</em> your cards and talk about them, but cards are
created and changed on the Neural page — chat has no card-writing tools.)</p>
<p>How a card comes to life:</p>
<ol>
<li><strong>You pick the source.</strong> A match from the small <strong>vetted catalog</strong> of free,
...
<strong>searches the web from your own words</strong> and offers up to three candidate
pages, each with evidence actually extracted from that page (<em>&quot;On the page:
High tide 7:12 AM&quot;</em>) so you can judge before you tap; the tap is the same
...
without asking you again.</li>
<li><strong>Built from real data.</strong> The engine samples the approved source and builds
...
off, say so on the card (<strong>Refine…</strong> or <em>&quot;Something&#39;s wrong&quot;</em>): your note
re-enters the build against the same frozen source, and a note that needs a
<em>different</em> source goes back through the approval step.</li>
<li><strong>Commissioning.</strong> The system then proves the card: a clean real run, your
one-tap confirmation that it&#39;s the <em>right</em> data, and one more clean run at
cadence to prove it&#39;s stable. Only then is the card live.</li>
<li><strong>It keeps itself honest.</strong> Every refresh is checked against the shape of the
...
broken card tells you in Chat.</li>
</ol>
<p>Cards render from a fixed set of safe building blocks (text, numbers, bars,
...
language-model step in its pipeline (below).</p>
<p><strong>Charts.</strong> The engine can keep a rolling history per card — a few numeric
series, a few hundred points each — and the card can draw it: a sparkline, a
gauge, or a change-since-last-refresh delta with its direction.</p>
<p><strong>Display rules and alerts.</strong> A card can carry small conditions. A display rule
(<em>&quot;turn the delta red when it goes negative&quot;</em>) is applied while the card is
prepared, so what you see is still a pure function of the data. An alert
(<em>&quot;tell me when the price drops&quot;</em>) fires a notice — but only when its condition
<em>becomes</em> true: it then stays quiet until the condition has been false again,
...
the badge on the Chat tab, and a durable copy on <strong>Info</strong> under <em>Neural
Interface</em>. On Mac and Linux the desktop app also shows a system notification.
While the vault is locked, nothing is shown anywhere.</p>
<p><strong>Interpreted cards.</strong> A card&#39;s pipeline can include one language-model step —
<em>&quot;summarize these headlines in a sentence&quot;</em>. It runs on your <strong>local</strong> model
...
card: a <strong>number the model reports must literally appear in the page&#39;s text</strong>
(an unverifiable figure is dropped, with a note in the card&#39;s history), and an
<strong>unchanged page skips the model entirely</strong> — no re-reading, no drift. Any
card whose content passed through a model — this step, or a card whose source
<em>is</em> a model instruction — wears an <strong>Interpreted</strong> chip, so you can always
tell a model&#39;s reading from pure arithmetic. Cards with a language-model step
refresh at most every 5 minutes.</p>
<p><strong>Self-repair.</strong> When a card fails because the data&#39;s shape changed, a local
...
in the card&#39;s run history. When a trial sticks, a <em>&quot;repaired itself&quot;</em> notice
tells you. Each card&#39;s <strong>Repair settings</strong> let you turn this off — and hold the
next level:</p>
<p><strong>Frontier repair (off by default).</strong> If local repair has had its one try and
the card still fails, and you have <em>both</em> switched on frontier repair for that
specific card <em>and</em> connected Claude Code, Claude can be asked for a second
opinion. Its answer is <strong>proposed, never applied</strong>: the card shows <em>&quot;Fix
proposed — review&quot;</em> with the current and proposed mappings side by side, and
your <strong>Apply</strong> runs it as the same keep-or-revert trial; <strong>Dismiss</strong> drops it.
What&#39;s sent is bounded — the card&#39;s goal, its data mappings, the failure, and a
short excerpt of the failing data; never your knowledge, never credentials.</p>
<p><strong>More than APIs.</strong> A card can also watch an ordinary <strong>web page</strong> (the
...
table, schema.org data, page metadata — the card is <em>compiled</em>: it reads the
exact cell or field you want on every refresh with no model involved, so its
values are the page&#39;s own. If the site later rearranges that structure, the card
re-picks the value itself on your local model (tried once, kept only if it works).
A page without usable structure gets an interpreted card instead. A card can
also watch your own <strong>knowledge</strong> (<em>&quot;a card of my notes
about X&quot;</em> — zero network), or an <strong>image</strong>: a weather-radar frame, a webcam
...
last good frame. <strong>Composite cards</strong> put your other cards&#39; histories side by
side — <em>&quot;my stock next to my spending&quot;</em> — with zero network: they read only
your own cards&#39; recorded series, can&#39;t nest, and degrade gracefully if a card
they reference goes away.</p>
<p><strong>Your MCP servers as sources.</strong> Cards can pull from MCP servers <strong>you</strong>
configure on <strong>Settings → Connections (MCP)</strong> — which is how database cards
...
public-addresses-only network guard. See <a href="#mcp">MCP</a>.</p>
<p><strong>The Library.</strong> Connect to a signed template library and install ready-made
cards. Trust works like vault subscriptions: the publisher&#39;s key is pinned on
first contact, every pack is verified against it, and the <strong>fingerprint</strong> is
...
Installing shows every source address up front and lands as a <strong>draft</strong>; your
<strong>Activate</strong> is still the consent — a library can never start traffic by
itself. When the library updates a template you use, the card offers <em>&quot;Update
available&quot;</em>: applying shows what changed and goes through draft → commissioning
...
because that content is yours.</p>
<p>Each card offers <strong>Run now</strong>, <strong>Pause</strong>, and <strong>Delete</strong>. Until a card is
commissioned it wears a preview badge that says where its data came from:
<em>&quot;real data, not updating yet&quot;</em> when the engine sampled your actual source,
or <em>&quot;sample data&quot;</em> when the preview was invented for look purposes (library
...
<p><strong>Safe search is always on</strong> — every engine is asked for its strict filtering
...
no setting to turn this off.</p>
<p>Searches happen when the assistant uses the web tools in a turn, and when a
...
<p>Everything you configure lives under <strong>Settings</strong>, as a row of tabs. It is Desktop-only: a
paired phone shows a note to make changes on the Desktop instead. Opening Settings lands on
<strong>Cloud providers</strong>; the tabs, in order:</p>
<h3 id="status">Status</h3>
<p>A live view of what the app is doing right now, refreshed every few seconds — the first
place to look when something seems off.</p>
<ul>
<li><strong>App</strong> — the version you are running and whether the vault is unlocked.</li>
<li><strong>Voice</strong> — the built-in dictation model (download progress, or <strong>Retry download</strong> if it
failed), which dictation engine is in use (<strong>built-in</strong> or <strong>your audio server</strong>), the
server voice if you set one, <strong>Playback speed</strong> for spoken replies (0.8× to 2×), your
<strong>Wake word</strong> with <strong>Test recognition</strong> and accepted spellings, the how-to-dictate
notes, and the <strong>Mic &amp; speaker check</strong>: <strong>Test now</strong> records three seconds with a level
bar, plays them back, and shows what dictation heard — the fastest way to tell a
microphone problem from a voice problem. See <a href="#voice">Voice</a>.</li>
<li><strong>Storage &amp; memory</strong> — disk used by everything SmartBrain stores (with the folder), by
the encrypted database and the voice model, the app&#39;s peak memory, and this browser&#39;s
cache.</li>
<li><strong>Model servers</strong> — whether Ollama, MLX, and MLX embeddings are configured (live
reachability is on the Local models tab).</li>
<li><strong>Knowledge</strong>, <strong>Schedules</strong>, <strong>Feeds</strong>, <strong>Remote access</strong> — document and embedded-chunk
counts, enabled schedules, feed subscriptions (with a <strong>failing</strong> count when a feed
can&#39;t be fetched), and paired devices.</li>
</ul>
<h3 id="cloud-providers">Cloud providers</h3>
<p>API keys for <strong>OpenAI</strong>, <strong>Anthropic</strong>, and <strong>Google (Gemini)</strong> — stored encrypted, never
shown back, replace or remove only. Saving a key discovers its chat models. Details:
<a href="#models__cloud-providers-your-api-keys">Connect a model</a>.</p>
<h3 id="local-models">Local models</h3>
<p>Optional servers on your own machines: <strong>Ollama</strong>, <strong>MLX</strong>, <strong>MLX embeddings</strong>, and the
<strong>Voice</strong> card (an optional audio server and server voice — the built-in dictation needs
none of this). Each card shows <strong>connected / unreachable / off</strong>, offers <strong>Connect it</strong> when
a server is detected on this machine, and has a <strong>Server on another machine</strong> option.
Details: <a href="#models__local-models-yours-on-this-machine-or-another-one-you-own">Local models</a>
and <a href="#models__voice-dictation-and-spoken-replies">Voice</a>.</p>
<h3 id="model-routing">Model routing</h3>
<p>Which model serves each job — chat, the agent (schedules and background tasks; <strong>Same as
Chat</strong> by default), embeddings for search — and <strong>Model context length</strong> per routed model.
Details: <a href="#models__which-model-does-what-model-routing">Which model does what</a>.</p>
<h3 id="web-search">Web search</h3>
<p>Which engine &quot;search the web&quot; uses: <strong>Automatic</strong>, <strong>SearXNG</strong> (your own instance),
<strong>Brave Search</strong>, <strong>Tavily</strong>, or <strong>DuckDuckGo</strong> (no key). Keys are stored encrypted.
Details: <a href="#web-search">Web search</a>.</p>
<h3 id="memory">Memory</h3>
<p>The assistant&#39;s name, your name, custom instructions, and the list of remembered facts
(<strong>Remember</strong> / <strong>Forget</strong>). Details: <a href="#memory">Memory</a>.</p>
<h3 id="self-improvement">Self-improvement</h3>
<p><strong>Self-review</strong> cadence (<strong>Off / 2h / 4h / 8h / 24h</strong>), the <strong>Prompt optimizer</strong> switch, and
the record of everything it proposed, applied, kept, or reverted. Both are off until you
turn them on. Details: <a href="#self-improvement">Self-improvement</a>.</p>
<h3 id="connections-mcp">Connections (MCP)</h3>
<p>The endpoint and access token that let a desktop AI client read your knowledge over MCP
— <strong>Generate</strong>, <strong>Copy</strong>, <strong>Regenerate</strong>, <strong>Revoke</strong>. Details: <a href="#mcp">MCP access</a>.</p>
<h3 id="remote-access">Remote access</h3>
<p><strong>Pair a new phone</strong> (a QR code that opens the app, plus an eight-character code that
expires) and the list of <strong>paired devices</strong> with <strong>Revoke</strong>. Details:
<a href="#remote-access">Remote access</a>.</p>
<h3 id="account-data">Account &amp; Data</h3>
<p><strong>Change passphrase</strong> (or set a new one after a Recovery-Key unlock), <strong>Export &amp; backup</strong>
(readable JSON export, or the full encrypted database), <strong>Chat trash</strong> (restore or empty;
30-day retention), and <strong>Restore</strong> from a backup file, applied on the next restart.
Details: <a href="#backup-recovery">Backup &amp; recovery</a>.</p>
<h2 id="next">Next</h2>
<ul>
...
`},{slug:`vaults`,title:`Share knowledge with Vaults`,headings:[{id:`next`,text:`Next`}],html:`<h1 id="share-knowledge-with-vaults">Share knowledge with Vaults</h1>
...
`},{slug:`mcp`,title:`Connect external tools (MCP)`,headings:[{id:`turn-it-on`,text:`Turn it on`},{id:`point-a-tool-at-it`,text:`Point a tool at it`},{id:`what-it-can-and-cant-do`,text:`What it can and can't do`},{id:`connect-out-your-mcp-servers-for-neural-interface-cards`,text:`Connect out: your MCP servers (for Neural Interface cards)`},{id:`next`,text:`Next`}],html:`<h1 id="connect-external-tools-mcp">Connect external tools (MCP)</h1>
...
<p>MCP runs in two directions here, and they are separate things: this page covers
the <strong>inbound</strong> server above first, then the <strong>outbound</strong> side — MCP servers
<em>you</em> run, feeding <a href="#features__neural-interface">Neural Interface</a> cards.</p>
...
<h2 id="connect-out-your-mcp-servers-for-neural-interface-cards">Connect out: your MCP servers (for Neural Interface cards)</h2>
<p>The other direction: SmartBrain can <strong>consume</strong> MCP servers as data sources for
<a href="#features__neural-interface">Neural Interface</a> cards — which is how database
cards work. Your own Postgres/SQLite MCP server keeps its credentials in its own
process; SmartBrain never holds the database password.</p>
<p>The rules, plainly:</p>
<ul>
<li><strong>Only servers you add explicitly.</strong> The lower half of <strong>Settings → Connections
(MCP)</strong> is where servers live — a label plus either a command SmartBrain launches
each run (<em>stdio</em>) or a URL you host (<em>http</em>). Adding, editing, and removing them
is Desktop-only. SmartBrain never reads ambient MCP config from disk (no
<code>.mcp.json</code> discovery, ever), and no assistant tool can create or change a server.</li>
<li><strong>Per card, one frozen tool call.</strong> A card names one server, one tool, and the
...
descriptions are never fed to a model; you pick the tool by name.</li>
<li><strong>Credentials stay in your server.</strong> A server config carries no secrets, and a
card&#39;s spec can&#39;t either — connection strings, passwords, and keys live in your
own server process, where SmartBrain never sees them.</li>
<li><strong>The one address exception, stated honestly.</strong> Everywhere else, SmartBrain&#39;s
...
connection elsewhere.</li>
<li><strong>Results are data, not instructions.</strong> Whatever the server returns is treated
like any other fetched content — bounded in size, checked against the card&#39;s
validated data shape, and rendered as plain text.</li>
</ul>
<p>Up to 10 servers can be configured. A server that&#39;s still referenced by a card
...
`},{slug:`backup-recovery`,title:`Backup & recovery`,headings:[{id:`export-your-data`,text:`Export your data`},{id:`encrypted-backup`,text:`Encrypted backup`},{id:`restore`,text:`Restore`},{id:`when-something-is-broken-what-to-try-in-order`,text:`When something is broken: what to try, in order`},{id:`starting-completely-fresh`,text:`Starting completely fresh`},{id:`chat-trash`,text:`Chat Trash`},{id:`change-your-passphrase`,text:`Change your passphrase`},{id:`forgot-your-passphrase`,text:`Forgot your passphrase?`},{id:`next`,text:`Next`}],html:`<h1 id="backup-recovery">Backup &amp; recovery</h1>
...
<strong>&quot;Forgot your current passphrase… Set a new one&quot;</strong> — enter your Recovery Key
again there, and it sets a new passphrase without the old one. (The normal
Change passphrase form still requires the current one.)</li>
</ol>
...
`},{slug:`privacy-security`,title:`Privacy & security`,headings:[{id:`what-protects-your-data`,text:`What protects your data`},{id:`what-leaves-your-machine-and-when`,text:`What leaves your machine (and when)`},{id:`honest-limits`,text:`Honest limits`},{id:`reporting-an-issue`,text:`Reporting an issue`}],html:`<h1 id="privacy-security">Privacy &amp; security</h1>
...
your network.</li>
<li><strong>Every request needs a credential.</strong> Being unlocked isn&#39;t enough on its own: each
...
and local self-repair run on a local model or not at all.) <strong>Building</strong> a card
...
later Fix or refine doesn&#39;t ask again) or with your local model.</li>
<li><strong>Claude Code models.</strong> The <code>claudecode/*</code> models send the conversation to Anthropic
under your own Claude sign-in, exactly like a cloud provider — configured beside the
local servers, but not local in the privacy sense. SmartBrain drives the <code>claude</code>
...
still goes to Anthropic. See <a href="#models__claude-code-sends-your-chats-to-anthropic">Connect a model</a>.</li>
...
or your own via <code>SMARTBRAIN_SIGNALING_URL</code>. It carries only connection metadata — the
...
who connected. See <a href="#remote-access">Remote access</a> for exactly what it sees.</li>
...
Recurring checks happen only if you turned auto-update on.</li>
<li><strong>Website feeds (only the ones you subscribed).</strong> Each RSS/Atom feed you follow is
fetched about every six hours, directly from this machine, from the public URL you
pasted — and from nowhere else. <strong>Refresh</strong> on a feed&#39;s row fetches right now.
Unsubscribing stops the checking.</li>
<li><strong>Neural Interface cards (only the sources you approved).</strong> Each card fetches its
...
only over HTTPS, and never follows a redirect while carrying it.</li>
<li><strong>Card creation, when no vetted source fits (a bounded search).</strong> While you are
<em>creating</em> a card and nothing in the vetted catalog matches, the build engine
searches <strong>your own words</strong> through the web-search engine below and reads up to
...
pre-read is part of the search; the address a card fetches <em>recurringly</em> is
still only the one you tap or paste. No search happens when a vetted source
matched, when you pasted a URL, or after the card exists.</li>
<li><strong>The card template library (only if you connect one).</strong> A connected library is
...
sent — it is the same file anyone can fetch.</li>
<li><strong>Your MCP servers (the deliberate localhost/LAN exception).</strong> The app&#39;s network
guard refuses private and internal addresses everywhere — with one exception,
stated plainly: an MCP server <strong>you</strong> add on Settings → Connections (MCP) may be
...
credentials stay inside your own server process and are never held by SmartBrain.</li>
<li><strong>Frontier card repair (only per-card, only if you opted in).</strong> If you switch on
&quot;Frontier repair via Claude&quot; for a specific card <em>and</em> Claude Code is connected,
...
is applied without your review.</li>
...
<p><strong>What sites see when SmartBrain fetches.</strong> Every fetch SmartBrain makes for you — card
sources, feeds, vault updates, the assistant&#39;s web search and page reads — comes from your
own IP address and identifies itself honestly as <code>SmartBrain/&lt;version&gt;</code> with a link to this
project; it does not pretend to be a web browser. So a site you point it at can tell that a
SmartBrain user, and which version, fetched it. Nothing else about you or your data is sent.</p>
<p>Two things that sound like they&#39;d leave and don&#39;t:</p>
<ul>
<li><strong>MCP (inbound).</strong> A connected desktop AI client reads your knowledge over a
loopback connection on your own machine. SmartBrain sends nothing outward for it.
What that <em>client</em> then does with what it read is its business, not SmartBrain&#39;s —
see <a href="#mcp">MCP</a>. (The <em>outbound</em> direction — your own MCP servers as card
sources — is listed above, because that one does connect out.)</li>
...
about it: outside text is marked as <em>data, not instructions</em> at the moment it enters
...
PDF text, hidden spreadsheet rows) is still read — that is why the gates exist.</li>
...
`},{slug:`remote-access`,title:`Remote access (away from home)`,headings:[{id:`how-it-works`,text:`How it works`},{id:`pair-your-phone`,text:`Pair your phone`},{id:`using-it-on-your-phone`,text:`Using it on your phone`},{id:`manage-devices`,text:`Manage devices`},{id:`security`,text:`Security`},{id:`on-your-own-wi-fi-lan-https`,text:`On your own Wi-Fi (LAN, HTTPS)`},{id:`self-hosting-the-signaling-node-advanced`,text:`Self-hosting the signaling node (advanced)`},{id:`next`,text:`Next`}],html:`<h1 id="remote-access-away-from-home">Remote access (away from home)</h1>
...
is <strong>content-blind</strong>: it never sees your data, only the connection setup. Be precise about what
that setup contains: the node reads the WebRTC offer and answer, which name the <strong>IP addresses</strong>
of your phone and your Desktop, and it knows <em>when</em> they are connected — nothing more. (Prefer
your own node? See <em>Self-hosting the signaling node</em> at the end.)</p>
...
<p><img src="assets/gifs/08-pair-a-phone.gif" alt="Pair a phone — QR plus a short pairing code over end-to-end-encrypted WebRTC"></p>
...
<strong>Pair a new phone</strong>. You&#39;ll see a QR code, three short steps, and an <strong>8-character code</strong> (shown as <code>ABCD-EFGH</code>).</p>
...
<li>In the installed app, <strong>enter the 8-character code</strong> (the dash is optional) and tap <strong>Pair</strong>.</li>
</ol>
...
<li><strong>End-to-end encrypted.</strong> The connection is encrypted (DTLS) between your phone and your
Desktop; the relay, when one is needed, carries only scrambled bytes.</li>
<li><strong>Identity-checked.</strong> Before sending anything, your phone verifies your Desktop&#39;s
identity: a <strong>key pinned at pairing</strong> (Ed25519), proven with a signature bound to <em>this</em>
...
nobody can quietly take its place while it is offline.</li>
<li><strong>One-time code.</strong> The 8-character pairing code (typed as <code>ABCD-EFGH</code>) is single-use,
expires in five minutes, and ends after eight wrong guesses — don&#39;t share it. The QR only
opens the site; it carries no secret.</li>
</ul>
<p><strong>What the node sees, and keeps.</strong> The node reads the connection offers and answers (your
phone&#39;s and Desktop&#39;s IP addresses, and when they connect), relays them, and holds nothing
else: it keeps <strong>no account</strong>, <strong>no access log</strong> (none is configured), writes <strong>no addresses
or ids</strong> into its application log, and its container logs are rotated at 10 MB. It never
holds your data, your device credential, or your pairing code. Relay credentials it hands
out expire after an hour and grant relay bandwidth only.</p>
<p><strong>Two honest limits.</strong></p>
<ol>
<li><em>Pairing trusts the node more than a session does.</em> During code pairing the node sees a
...
node you run yourself.</li>
<li><em>The phone app is served from the node&#39;s domain.</em> Whoever runs the node ships the
...
to you, and using the Desktop alone avoids it.</li>
</ol>
...
<li><p><strong>On the phone (same Wi-Fi)</strong> open <code>https://192.168.1.50:33000</code> and enter your
passphrase once (again after each app restart).</p>
</li>
</ol>
<p>A browser that reaches the app at a LAN address gets a phone&#39;s permissions, not the
...
itself, keep using <code>https://localhost:33000</code>. This keeps your phone&#39;s browser to a phone&#39;s
permissions; it isn&#39;t a barrier against someone on your network who knows your passphrase
and uses their own tools.</p>
<blockquote>
...
which id (<code>SIGNALING_STATE_FILE</code>, a small JSON file on a volume) so an id can&#39;t be
taken over. Per-address and per-socket limits are on by default.</p>
</li>
...
Only <code>wss://</code> is accepted (an unencrypted <code>ws://</code> node is refused). Then pair devices as
above. The node&#39;s health is probed every 30 seconds, and <code>deploy/sb-autoheal.*</code> restarts
a container that reports unhealthy — install those units alongside the compose file.</p>
</li>
</ol>
...
`},{slug:`design-limits`,title:`Design limits`,headings:[{id:`single-user-global-unlock`,text:`Single-user global unlock`},{id:`single-writer-embedded-database-duckdb`,text:`Single-writer embedded database (DuckDB)`},{id:`no-key-at-rest-restart-returns-to-locked`,text:`No key at rest (restart returns to locked)`},{id:`append-only-audit-log-no-hash-chain`,text:`Append-only audit log (no hash chain)`},{id:`the-search-index-lives-in-memory-rebuilt-on-each-unlock`,text:`The search index lives in memory (rebuilt on each unlock)`},{id:`one-local-model-request-at-a-time`,text:`One local-model request at a time`},{id:`voice-one-dictation-at-a-time-120-seconds-one-download`,text:`Voice: one dictation at a time, 120 seconds, one download`},{id:`neural-interface-bounded-cards-bounded-refresh`,text:`Neural Interface: bounded cards, bounded refresh`},{id:`a-turn-is-bounded`,text:`A turn is bounded`},{id:`webrtc-signaling-broker-is-single-operator`,text:`WebRTC signaling broker is single-operator`},{id:`next`,text:`Next`}],html:`<h1 id="design-limits">Design limits</h1>
...
with your passphrase, the launcher&#39;s token, or your paired phone.</p>
...
<h2 id="neural-interface-bounded-cards-bounded-refresh">Neural Interface: bounded cards, bounded refresh</h2>
<p>The card dashboard is sized for a person, not a fleet, and every bound is a
number you can check:</p>
<ul>
<li><strong>200 cards</strong> per vault, and the engine refreshes <strong>at most 3 due cards per
pass</strong> (a pass rides the same ~30-second background tick as feeds and
schedules, with its own time budget) — so a screenful of cards drains
steadily without ever crowding out your chat or your schedules.</li>
<li><strong>1 minute is the cadence floor</strong> — cards are near-live, never real-time. A
card with a language-model step in its pipeline floors at <strong>5 minutes</strong>
instead, and only one such card runs per pass: the local model is a single
slot, and your chat has priority on it.</li>
<li><strong>An alert&#39;s cooldown is at least 5 minutes</strong> (an hour unless you set one).
Alerts are edge-triggered on top of that — a condition that stays true fires
once, not every refresh.</li>
<li><strong>At most 10 MCP servers</strong> can be configured as card sources, with one bounded
tool call per refresh.</li>
<li><strong>A library pack is at most 2 MB and 200 templates</strong>, checked about once a
day against the one URL you pinned.</li>
<li><strong>An image card&#39;s picture is at most 4 MB</strong> per fetch, and only real raster
formats (checked by file signature) are accepted.</li>
</ul>
<p><strong>Why:</strong> a dashboard that watches things for you is a standing invitation to
...
refusal at a bound is reported on the card rather than silently truncated.</p>
...
<strong>single-operator</strong> by design: one node, in-memory sessions, no failover. The
hosted broker is <strong>tokenless</strong> (open registration — but every Desktop proves it
...
cryptographic guarantee that your phone is really talking to <strong>your</strong> Desktop is
the <strong>Ed25519 key pinned at pairing</strong>, proven per connection with a signature bound
to that connection&#39;s DTLS fingerprints — not the broker. TURN relay uses
<strong>ephemeral credentials</strong> (coturn <code>use-auth-secret</code>, minted per connection, one-hour
lifetime); those credentials grant <strong>relay bandwidth only</strong>, never access to the
app or your data. A <strong>self-hosted</strong> node may instead run with a shared registration
token and static, quota-bounded TURN creds.</p>
...
`}],w=a(`<li><a> </a></li>`),T=a(`<ul class="help-sub svelte-1vby5nc"></ul>`),E=a(`<a> </a> <!>`,1),D=a(`<div class="help svelte-1vby5nc"><nav class="help-nav svelte-1vby5nc" aria-label="Help sections"><h2 class="svelte-1vby5nc">Help</h2> <!></nav> <article class="help-body card svelte-1vby5nc"></article></div>`);function O(a,O){_(O,!0);let k=y(()=>S.url.hash.replace(/^#/,``).split(`__`)),A=y(()=>C.find(e=>e.slug===c(k)[0])??C[0]),j=y(()=>c(k)[1]),M=s(void 0);u(()=>{if(c(A),typeof window>`u`||!c(M))return;let e=window.matchMedia(`(prefers-reduced-motion: reduce)`);for(let e of c(M).querySelectorAll(`pre`))e.tabIndex=0;let t=()=>{for(let t of c(M).querySelectorAll(`img`)){let n=t.getAttribute(`src`)??``;e.matches&&n.endsWith(`.gif`)?(t.dataset.gif=n,t.setAttribute(`src`,n.replace(/\.gif$/,`.poster.png`))):!e.matches&&t.dataset.gif&&(t.setAttribute(`src`,t.dataset.gif),delete t.dataset.gif)}};return t(),e.addEventListener(`change`,t),()=>e.removeEventListener(`change`,t)}),u(()=>{c(A);let e=c(j);typeof window>`u`||!c(M)||!e||!/^[\w-]+$/.test(e)||c(M).querySelector(`[id="${e}"]`)?.scrollIntoView()});var N=D(),P=i(N),F=d(i(P),2);t(F,17,()=>C,e=>e.slug,(a,s)=>{var l=E(),u=v(l);let f;var p=r(u,!0),m=d(u,2),_=a=>{var o=T();t(o,21,()=>c(s).headings,e=>e.id,(t,a)=>{var o=w(),l=i(o);let u;var d=r(l,!0);b(o),n(()=>{u=x(l,1,`help-sublink svelte-1vby5nc`,null,u,{active:c(j)===c(a).id}),h(l,`href`,`#${c(s).slug}__${c(a).id}`),g(d,c(a).text)}),e(t,o)}),b(o),n(()=>h(o,`aria-label`,`Sections of ${c(s).title}`)),e(a,o)};o(m,e=>{c(s).slug===c(A).slug&&c(s).headings.length>1&&e(_)}),n(()=>{f=x(u,1,`help-link svelte-1vby5nc`,null,f,{active:c(s).slug===c(A).slug}),h(u,`aria-current`,c(s).slug===c(A).slug?`page`:void 0),h(u,`href`,`#${c(s).slug}`),g(p,c(s).title)}),e(a,l)}),b(P);var I=d(P,2);l(I,()=>c(A).html,!0),b(I),m(I,e=>f(M,e),()=>c(M)),b(N),e(a,N),p()}export{O as component};