import{A as e,B as t,C as n,D as r,I as i,J as a,K as o,O as s,V as c,W as l,Y as u,Z as d,b as f,c as p,m,nt as h,ot as g,tt as _,w as v}from"../chunks/BKwOMm0i.js";import"../chunks/xihTtKlq.js";import{t as y}from"../chunks/yFJDnUbT.js";var b=[{slug:`getting-started`,title:`Getting started`,html:`<h1 id="getting-started">Getting started</h1>
...
force it.</p>
<p><strong>Linux (native)</strong> updates itself the same way. On a headless install the swap happens
under systemd: the launcher installs the new version, exits, and the unit&#39;s <code>Restart=</code>
brings the new one up.</p>
...
<p>Chat can listen and talk — see <a href="#features__voice-dictate-and-listen">Voice in Chat</a>
for how it feels to use. Under the hood, <strong>your voice never leaves your machines</strong>:
speech-to-text runs on a local audio server you configure under <strong>Settings → Local
models → Voice</strong>, and replies are spoken with your device&#39;s own voices.</p>
<ul>
<li><strong>Mac (Apple Silicon)</strong> — <strong>oMLX already serves the audio endpoints</strong>; leave the
Voice address empty and SmartBrain uses your MLX server. One thing to do once:
<strong>load a whisper model into oMLX</strong> (for example
<code>mlx-community/whisper-large-v3-turbo</code>), the same way you loaded your chat model —
...
if the server is reachable but has no transcription model yet.</li>
<li><strong>Windows / Linux</strong> — run any local server that speaks the standard
<code>/v1/audio/transcriptions</code> API and put its address in the Voice card. Two good
choices: <a href="https://github.com/speaches-ai/speaches">speaches</a> (CPU or GPU, also
offers server voices) or <code>whisper.cpp</code>&#39;s server, each with a whisper model pulled.
SmartBrain auto-detects whatever whisper model the server offers — you only set a
model name if you want a specific one.</li>
...
with the server above. Replies use the phone&#39;s own voices, offline.</li>
</ul>
...
there (or if you just want a nicer voice), set the optional <strong>Server voice model</strong> in
the Voice card (e.g. <code>kokoro</code> on a server that offers speech) and SmartBrain speaks
...
<h3 id="voice-dictate-and-listen">Voice — dictate and listen</h3>
<p>Chat can listen and talk. If a voice server is configured (see
<a href="#models__voice-dictation-and-spoken-replies">Voice</a> — on a Mac, oMLX with a
whisper model loaded is all it takes), a <strong>microphone button</strong> appears in the message box: tap, speak, tap
again, and your words land in the box as text — <strong>you review before it sends</strong>. The
transcription happens on your own machine; nothing is sent to any speech service.</p>
<p>The <strong>speaker button</strong> next to it turns on spoken replies: answers are read aloud
<strong>sentence by sentence as they stream in</strong>, in your device&#39;s own voice. Every answer
also has a quiet <strong>Listen</strong> action underneath to hear just that one. Speaking to the
assistant — or asking something new — interrupts the voice mid-sentence, the way a
conversation should.</p>
<p>On the phone, both work the same way: dictation travels the encrypted connection to
...
<p>Your Desktop must be <strong>running and unlocked</strong> for any of this to work — and since the
phone is a window onto the Desktop, they share one vault and one lock: unlock (or lock)
on either, and both follow. The phone is a window onto it, not a copy of it.</p>
