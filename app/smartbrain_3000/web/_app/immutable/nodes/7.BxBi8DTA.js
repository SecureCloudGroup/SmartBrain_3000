import{$ as e,A as t,B as n,C as r,D as i,F as a,G as o,J as s,O as c,U as l,Y as u,at as d,b as f,c as p,et as m,m as h,q as g,w as _,z as v}from"../chunks/CxCnSy1g.js";import"../chunks/xihTtKlq.js";import{t as y}from"../chunks/DhOu0D01.js";var b=[{slug:`getting-started`,title:`Getting started`,html:`<h1 id="getting-started">Getting started</h1>
...
<p><strong>The desktop app updates itself — no commands.</strong> The menu-bar launcher quietly checks for a newer
version in the background; when one is ready it shows <strong>Install update now</strong> and <strong>Install on next
start</strong> in the menu. Click <strong>Install update now</strong> to apply it immediately, or just keep working and
...
A <strong>paired phone</strong> needs nothing either: the phone app refreshes itself the next time you open it.</p>
<p>The menu-bar app itself (the small launcher, not the app) updates through however you installed it —
<strong>Homebrew:</strong> <code>brew update &amp;&amp; brew upgrade --cask smartbrain</code>; <strong>Scoop:</strong> <code>scoop update smartbrain</code> —
but that&#39;s rarely needed, since the app that matters updates on its own.</p>
...
<p>Local models keep every prompt on your hardware — nothing goes to a provider. They run on
the <strong>host</strong> (not inside the container), and the app reaches them at <code>host.docker.internal</code>.
...
path on a Mac, so it&#39;s the one to reach for first there. Install <code>mlx-lm</code>
(<code>pip install mlx-lm</code>) and start a server for a model:</p>
<pre><code class="language-sh">mlx_lm.server --host 0.0.0.0 --port 8888 --model mlx-community/Qwen2.5-7B-Instruct-4bit
</code></pre>
</li>
<li><p><strong>Ollama</strong> — works on <strong>any OS</strong> (macOS, Linux, Windows). <a href="https://ollama.com/download">Install it</a>,
then pull a model:</p>
...
your knowledge content stays on-box. The same embedding model also runs on <strong>MLX</strong>, so an
Apple-Silicon Mac can run the whole stack MLX-only — point embeddings at your MLX server if
you&#39;re not running Ollama.</p>
...
<strong>Rename</strong> a saved chat.</p>
...
`}],x=t(`<a> </a>`),S=t(`<div class="help svelte-1vby5nc"><nav class="help-nav svelte-1vby5nc" aria-label="Help sections"><h2 class="svelte-1vby5nc">Help</h2> <!></nav> <article class="help-body card svelte-1vby5nc"></article></div>`);function C(t,C){m(C,!0);let w=u(()=>y.url.hash.replace(/^#/,``).split(`__`)),T=u(()=>b.find(e=>e.slug===a(w)[0])??b[0]),E=u(()=>a(w)[1]),D=s(void 0);n(()=>{if(a(T),typeof window>`u`||!a(D))return;let e=window.matchMedia(`(prefers-reduced-motion: reduce)`),t=()=>{for(let t of a(D).querySelectorAll(`img`)){let n=t.getAttribute(`src`)??``;e.matches&&n.endsWith(`.gif`)?(t.dataset.gif=n,t.setAttribute(`src`,n.replace(/\.gif$/,`.poster.png`))):!e.matches&&t.dataset.gif&&(t.setAttribute(`src`,t.dataset.gif),delete t.dataset.gif)}};return t(),e.addEventListener(`change`,t),()=>e.removeEventListener(`change`,t)}),n(()=>{a(T);let e=a(E);typeof window>`u`||!a(D)||!e||!/^[\w-]+$/.test(e)||a(D).querySelector(`[id="${e}"]`)?.scrollIntoView()});var O=S(),k=l(O);_(o(l(k),2),17,()=>b,e=>e.slug,(e,t)=>{var n=x();let r;var o=l(n,!0);d(n),v(()=>{r=f(n,1,`help-link svelte-1vby5nc`,null,r,{active:a(t).slug===a(T).slug}),h(n,`aria-current`,a(t).slug===a(T).slug?`page`:void 0),h(n,`href`,`#${a(t).slug}`),i(o,a(t).title)}),c(e,n)}),d(k);var A=o(k,2);r(A,()=>a(T).html,!0),d(A),p(A,e=>g(D,e),()=>a(D)),d(O),c(t,O),e()}export{C as component};