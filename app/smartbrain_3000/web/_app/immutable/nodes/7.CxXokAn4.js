...
<p>On <strong>macOS and Windows</strong>: nothing. There is no Docker to install, no Python, no accounts,
...
against a checksum, and runs them as two ordinary programs on your machine.</p>
<p>Two cases need <a href="https://docs.docker.com/get-docker/">Docker</a> installed and running:</p>
<ul>
<li><strong>Linux</strong> — there is no desktop app for Linux yet, so it runs the Docker stack.</li>
...
<p><strong>Linux</strong> — no desktop app yet. Download the release compose file and start it:</p>
...
<p>Open <strong><a href="http://localhost:33000">http://localhost:33000</a></strong> and complete first-run setup below. Your data lives in
named Docker volumes; back it up with the in-app encrypted backup.</p>
...
<ul>
<li><strong>In SmartBrain itself</strong>, a strip at the top of the page: <em>&quot;SmartBrain vX.Y.Z is ready to
...
<p>On <strong>Linux</strong> and <strong>Intel Macs</strong>, where SmartBrain runs in Docker, two more apply:</p>
<ul>
...
Windows, <code>docker compose -f docker-compose.release.yml down</code> on Linux. From source,
<code>docker compose down</code> in <code>compose/</code>.</p>
...
<p>On Linux the data is in Docker volumes, so it goes with the stack:
...
and use <a href="#local-models-on-your-machine">a local model</a> instead.</p>
...
<p>The page also lists <strong>Fast chat</strong> and <strong>Reasoning</strong> slots. Nothing in the app requests them
today, so setting them changes nothing.</p>
...
<p>The sidebar (a bottom tab bar on a phone) holds nine areas. This is the whole app:</p>
<table>
<thead>
<tr>
<th>Area</th>
...
<td>What your cloud models have cost. Desktop only.</td>
</tr>
<tr>
<td><strong>Settings</strong></td>
<td>Everything you configure. Desktop only.</td>
</tr>
</tbody></table>
...
(<a href="#remote-access">Remote access</a>) you get the seven meant for use on the go — Chat,
...
<p>If the conversation has scrolled, a <strong>Jump to latest</strong> pill brings you back down.</p>
<h3 id="after-an-answer">After an answer</h3>
<p>Every reply carries <strong>Copy</strong> (the raw Markdown, not the rendered page). The most recent one
also offers <strong>Regenerate</strong> — ask again for a fresh answer to your last message. The new
...
<tr>
<td>Update a task</td>
<td>Edits a task&#39;s title, date, priority, tags or notes.</td>
</tr>
<tr>
...
<p><strong>Each row also has Rename and Delete</strong>, and a checkbox for picking documents to put in a
vault. Renaming re-indexes the document in the background, because the title is part of what
search matches on; tagging doesn&#39;t.</p>
...
<p>The page has two tabs. <strong>Create</strong> takes a name, the prompt itself, how often it should
...
the price list from the on-device gateway. This page is Desktop-only.</p>
...
<ul>
<li><strong>Awaiting your approval</strong> — a card per proposed action, naming the tool, what it would
do, and whether it is reversible. <strong>Approve</strong> or <strong>Deny</strong> it. <strong>Always allow</strong> approves it
...
tools can&#39;t be pre-authorized — they ask every time, with an extra confirmation.</li>
<li><strong>History</strong> — the record of every tool the assistant ran or tried to run: which tool, its
...
secrets are stripped before anything is recorded.</li>
</ul>
...
label it.</p>
<p><strong>Reading the card.</strong> A vault card carries chips for whatever applies: <strong>Public</strong> with your
publisher fingerprint and the version you last exported; <strong>Subscribed</strong> with the pinned
publisher fingerprint, the host it came from, and the version you have; <strong>Imported</strong> for one
that arrived as a file. Checking a subscription tells you where you stand — <em>&quot;Up to date
(v3).&quot;</em> or <em>&quot;Update available (v3 → v4).&quot;</em></p>
...
<li>After staging, restart SmartBrain — <strong>Restart</strong> in the menu-bar / tray menu, or
<code>docker compose -f docker-compose.release.yml restart</code> on Linux (from source,
<code>python3 installer/install.py update</code>) — and unlock with that backup&#39;s passphrase.</li>
...
<strong>Knowledge</strong>, <strong>Planner</strong>, <strong>Schedules</strong>, <strong>Email</strong>, <strong>Info</strong>, and <strong>Activity</strong>. Usage,
Settings, and first-time setup live on the <strong>Desktop</strong>.</p>
<p>A handful of individual actions are Desktop-only too, even inside those areas — anything
...
or replaced with a line pointing you at the Desktop, so nothing fails halfway.</p>
...
<p>Your Desktop must be <strong>running and unlocked</strong> for any of this to work. The phone is a window
onto it, not a copy of it.</p>
...
`}],x=e(`<a> </a>`),S=e(`<div class="help svelte-1vby5nc"><nav class="help-nav svelte-1vby5nc" aria-label="Help sections"><h2 class="svelte-1vby5nc">Help</h2> <!></nav> <article class="help-body card svelte-1vby5nc"></article></div>`);function C(e,C){h(C,!0);let w=d(()=>y.url.hash.replace(/^#/,``).split(`__`)),T=d(()=>b.find(e=>e.slug===i(w)[0])??b[0]),E=d(()=>i(w)[1]),D=u(void 0);c(()=>{if(i(T),typeof window>`u`||!i(D))return;let e=window.matchMedia(`(prefers-reduced-motion: reduce)`);for(let e of i(D).querySelectorAll(`pre`))e.tabIndex=0;let t=()=>{for(let t of i(D).querySelectorAll(`img`)){let n=t.getAttribute(`src`)??``;e.matches&&n.endsWith(`.gif`)?(t.dataset.gif=n,t.setAttribute(`src`,n.replace(/\.gif$/,`.poster.png`))):!e.matches&&t.dataset.gif&&(t.setAttribute(`src`,t.dataset.gif),delete t.dataset.gif)}};return t(),e.addEventListener(`change`,t),()=>e.removeEventListener(`change`,t)}),c(()=>{i(T);let e=i(E);typeof window>`u`||!i(D)||!e||!/^[\w-]+$/.test(e)||i(D).querySelector(`[id="${e}"]`)?.scrollIntoView()});var O=S(),k=l(O),A=o(l(k),2);v(A,17,()=>b,e=>e.slug,(e,n)=>{var a=x();let o;var c=l(a,!0);g(a),t(()=>{o=f(a,1,`help-link svelte-1vby5nc`,null,o,{active:i(n).slug===i(T).slug}),m(a,`aria-current`,i(n).slug===i(T).slug?`page`:void 0),m(a,`href`,`#${i(n).slug}`),r(c,i(n).title)}),s(e,a)}),g(k);var j=o(k,2);n(j,()=>i(T).html,!0),g(j),p(j,e=>a(D,e),()=>i(D)),g(O),s(e,O),_()}export{C as component};